...
Created on 2024-07-05 13:46:28 +0300.
...
A0 = np.array([[cos_a, -(1 / KAPPA) * sin_a],
               [KAPPA * sin_a, cos_a]])
B0 = np.array([[cos_a, -KAPPA * sin_a],
               [(1 / KAPPA) * sin_a, cos_a]])
...
LAMBDA = (3 + 2 * KAPPA**2 + 3 * KAPPA**4 + np.sqrt(3) * (KAPPA**2 - 1)
          * np.sqrt(3 + 10 * KAPPA**2 + 3 * KAPPA**4)) / (8 * KAPPA**2)
...
dx = [0.12, 0.12, 0.12, 0.0, 0.0, -0.1,
      -0.1, -0.1, -0.12, 0.0, 0.0, 0.13]
dy = [0.0, 0.0, 0.0, -0.08, -0.08, 0.0,
...
adx = [0.08, 0.0, -0.04, -0.06, -0.1, -0.1,
       -0.08, -0.04, 0.08, 0.1, 0.13, 0.14]
ady = [-0.06, -0.08, -0.08, -0.09, 0.0, 0.04,
       0.06, 0.1, 0.1, 0.12, 0.0, 0.0]
for i, label in enumerate(labels):
...
bdx = [0.1, 0.04, -0.02, -0.06, -0.1, -0.1,
       -0.1, -0.04, 0.0, 0.1, 0.13, 0.12]
bdy = [-0.08, -0.1, -0.1, -0.08, 0.0, 0.05,
       0.06, 0.1, 0.1, 0.06, 0.0, -0.06]
for i, label in enumerate(labels):
