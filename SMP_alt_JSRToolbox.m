...
A0 = [cos_a, -(1 / kappa) * sin_a; kappa * sin_a, cos_a];
B0 = [cos_a, -kappa * sin_a; (1 / kappa) * sin_a, cos_a];
...
adx = [-0.075, -0.1, 0.0, 0.0, 0.025, 0.025, ...
    0.025, 0.0, -0.075, -0.1, -0.125, -0.125];
ady = [0.05, 0.05, 0.05, 0.05, 0.0, 0.0, ...
...
bdx = [-0.125, -0.05, -0.02, 0.0, 0.025, 0.025, ...
    0.05, 0.0, -0.04, -0.1, -0.125, -0.125];
bdy = [0.05, 0.05, 0.06, 0.05, 0.0, -0.05, ...
    -0.05, -0.06, -0.06, -0.05, 0.0, 0.025];
text(BVx+bdx, BVy+bdy, sss, 'Interpreter', 'latex');
grid on
pbaspect([1, 1, 1]);
axis([-1.1, 1.1, -1.1, 1.1]);

exportgraphics(gcf, 'SMP-alt-JSRToolbox.pdf', 'Resolution', 1200);
